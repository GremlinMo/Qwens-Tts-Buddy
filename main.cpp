...
constexpr wchar_t AppName[] = L"Qwens Tts Buddy";
constexpr wchar_t AppVersion[] = L"1.0.0";
constexpr wchar_t AppVersionLabel[] = L"v1.0.0";
...
HWND Edit(int id, const wchar_t* t, int x, int y, int w, int h, HWND p, bool multi = false) {
    ...
    return New(L"EDIT", t, style, id, x, y, w, h, p);
}
...
void SetActiveJobProcess(HANDLE h) {
    std::lock_guard<std::mutex> lock(g.jobMutex);
    g.jobProcess = h;
}

void ClearActiveJobProcess(HANDLE h) {
    std::lock_guard<std::mutex> lock(g.jobMutex);
    if (g.jobProcess == h) g.jobProcess = nullptr;
}
...
void CancelActiveJob() {
    ...
    if (h) {
        TerminateProcess(h, 1223);
        SetStatus(L"Cancelling generation...");
        Log(L"Generation cancel requested. Waiting for Qwen backend to close...\r\n");
    } else {
        SetStatus(L"Cancel requested...");
        Log(L"Cancel requested before backend process was ready.\r\n");
    }
}

DWORD RunProcess(const std::wstring& cmd, const std::wstring& cwd, std::wstring& out, bool trackJob = false) {
...
void RunJob(const std::string& json) {
    ...
    SetWindowTextW(g.h[IDC_STOP], L"Cancel");
    Text(g.h[IDC_LOG], L"");
    ...
    std::thread([json] {
        ...
        if (g.cancelRequested) {
            out = L"Generation cancelled before backend started.\r\n";
        } else {
            code = RunProcess(Quote(python) + L" " + Quote(py) + L" " + Quote(cfg), QwenRoot, out, true);
            out = CleanBackendOutput(out);
        }
        if (g.cancelRequested) {
...
void CreateUi(HWND hwnd) {
    ...
    Button(IDC_NAV_GENERATE, L"Generate", 18, 116, 150, 36, hwnd);
    Button(IDC_NAV_LIBRARY, L"Voice Library", 18, 160, 150, 36, hwnd);
    Button(IDC_NAV_CLONE, L"Voice Clone", 18, 204, 150, 36, hwnd);
    Button(IDC_NAV_FINETUNE, L"Fine Tune", 18, 248, 150, 36, hwnd);
    Button(IDC_NAV_MODELS, L"Models", 18, 292, 150, 36, hwnd);
    Button(IDC_NAV_SETTINGS, L"Settings", 18, 336, 150, 36, hwnd);
    ...
    T(PAGE_SETTINGS, Label((std::wstring(L"App version ") + AppVersionLabel).c_str(), 210, 64, 260, 24, hwnd));
...
void Paint(HWND hwnd) {
    ...
    TextOutW(dc, 18, 28, AppName, (int)wcslen(AppName));
    SelectObject(dc, g.font);
    SetTextColor(dc, g.c.muted);
    TextOutW(dc, 18, 60, AppVersionLabel, (int)wcslen(AppVersionLabel));
    TextOutW(dc, 18, 80, L"Offline voice workbench", 23);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_COMMAND:
        ...
        case IDC_STOP:
            if (g.running) CancelActiveJob();
            else PlayAudio(L"stop");
            return 0;
    ...
    case WM_APP + 1: {
        ...
        delete s;
        bool cancelled = g.cancelRequested.load();
        g.running = false;
        g.cancelRequested = false;
        EnableWindow(g.h[IDC_GENERATE], TRUE);
        EnableWindow(g.h[IDC_CLONE_GENERATE], TRUE);
        SetWindowTextW(g.h[IDC_STOP], L"Stop");
        SetStatus(cancelled ? L"Cancelled" : (wp == 0 ? L"Complete" : L"Failed - see log"));
        ScanModels();
        RefreshLibrary();
        return 0;
    }
...
int WINAPI wWinMain(HINSTANCE inst, HINSTANCE, PWSTR, int show) {
    ...
    RegisterClassExW(&wc);
    std::wstring windowTitle = std::wstring(AppName) + L" " + AppVersionLabel;
    HWND hwnd = CreateWindowExW(0, wc.lpszClassName, windowTitle.c_str(), WS_OVERLAPPEDWINDOW | WS_VISIBLE,
